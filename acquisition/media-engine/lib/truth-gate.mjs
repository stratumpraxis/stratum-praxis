// Issue #52 - the first-person truth gate, the fabricated-biography gate and the
// privacy gate.
//
// The rule this file enforces: if a sentence requires a personal fact that the identity
// contract has not explicitly approved, the sentence does not get written. It is not
// softened, hedged or attributed to a persona - it is rejected and a safe rewrite is
// named.
//
// English and Spanish patterns sit side by side on purpose. A fabrication that would be
// blocked on the English desk must not survive by being written in Spanish.

import { isApprovedClaim, safeRewriteFor } from './identity.mjs';

/**
 * Each family maps to a claim_id in identity.json. A match is a violation unless that
 * claim_id is in approved_first_person_claims - and even then only within its scope.
 */
export const FIRST_PERSON_CLAIMS = Object.freeze([
  {
    ...
    patterns: [
      ...
      /\b(?:prob[ée]|he\s+probado|test[ée]|he\s+testeado)\b/i,
  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(?:compr[ée]|pagu[ée]|me\s+suscrib[ií])\b/i
    ]
  },
  {
    claim_id: 'daily_use',
  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(?:visit[ée]|viaj[ée]|estuve\s+en)\b/i
    ]
  },
  {
  ...
  },
  {
    ...
    patterns: [
      ...
      /\b(?:fund[ée]|vend[ií]|renunci[ée]|estudi[ée])\b/i,
      /\b(?:constru[ií]|lanc[ée]|cre[ée])\b/i
    ],
    // These verbs are legitimate when the sentence is about an asset the operator
    // actually publishes, which is the approved operates_these_tools claim.
    approved_when: /stratumpraxis|the\s+(?:calculator|checklist|tool|page)\s+I\s+publish|que\s+publico/i,
    approved_claim_id: 'operates_these_tools'
  }
]);

/** Country-level residence is approved; anything narrower is not. */
export const LOCATION_CLAIM = Object.freeze({
  ...
  approved_values: ['japan', 'japon', 'japón']
});
...
export function checkTruth(text, { identity, source = null, lens = null, field = 'body' } = {}) {
  ...
  for (const sentence of sentences) {
    for (const family of FIRST_PERSON_CLAIMS) {
      ...
      if (family.approved_when && family.approved_when.test(sentence)
...
export function checkLocationScope(text, identity, field = 'body') {
  ...
  for (const pattern of [LOCATION_CLAIM.pattern, LOCATION_CLAIM.spanish]) {
    ...
    for (const match of String(text || '').matchAll(regex)) {
      ...
      if (LOCATION_CLAIM.approved_values.includes(place)) continue;
      violations.push({
        gate: 'PRIVACY_LOCATION_SCOPE',
        claim_id: 'residence_history',
        label: 'location narrower than country',
        field,
        sentence: match[0],
        matched: place,
        safe_rewrite: 'State the country only: "Japan-based" / "desde Japon".'
      });
    }
  }
  return violations;
...
export function checkPrivacy(text, identity, field = 'body') {
  ...
  for (const rule of PRIVACY_PATTERNS) {
    ...
    if (match) {
      violations.push({
        ...
        sentence: match[0],
        matched: rule.id,
        safe_rewrite: 'Remove entirely. This engine never publishes private or credential material.'
      });
    }
  }

  const allowed = new Set((identity.privacy_redactions?.public_contact_allowlist || []).map((e) => e.toLowerCase()));
  for (const match of body.matchAll(EMAIL_PATTERN)) {
    ...
    violations.push({
      ...
      sentence: match[0],
      matched: match[0],
      safe_rewrite: 'Remove the address, or use the contact already published on the public site.'
    });
  }
  return violations;
}

/** Lens-specific bans, including the Japan generalisation rule. */
export function checkLensRules(text, lens, field = 'body') {
  ...
  if (lens.lens_id === 'japan_reality') {
    const generalisation = /\b(?:Japan\s+is|Japanese\s+(?:people|workers|companies)\s+(?:are|all)|In\s+Japan,?\s+everyone)\b/i;
    for (const sentence of splitSentences(body)) {
      if (!generalisation.test(sentence)) continue;
      // A scoped or sourced sentence is fine; a bare national claim is not.
      ...
      violations.push({
        gate: 'LENS_CONTRACT',
        claim_id: 'unsupported_national_generalisation',
        label: 'national generalisation without evidence or scope',
        field,
        sentence,
        matched: 'national generalisation',
  ...
  for (const restricted of source.restricted_claims || []) {
    ...
    if (body.includes(phrase)) {
      violations.push({
        ...
        matched: phrase,
        safe_rewrite: restricted.safe_rewrite || 'This claim is not supported by the approved source material.'
      });
    }
  }
  return violations;
...
/** Run the gate across every text field of a derivation. */
export function checkDerivation(derivation, context) {
  const fields = [
    ['title', derivation.title],
    ['hook', derivation.hook],
    ['body', derivation.body],
    ['cta_text', derivation.cta_text]
  ];
  const violations = [];
  let sentences = 0;
