...
export function buildRules(identity, source, lens, vertical, { compact = false } = {}) {
  ...
  if (compact) {
    ...
    return [
      ...
      'HARD RULES: no statistic or count that is not in ALLOWED CLAIMS unless the sentence labels it as an example; NEVER write a currency amount that is not in ALLOWED CLAIMS, not even inside an example - describe price relationships in words ("the lower tier", "a fraction of the seat price"); every source figure carries its provenance in the same sentence; no invented email, routine, client, purchase, test, conversation or outcome; at most one heading per 300 words and at most two coined capitalised terms; no closing motivational paragraph and no summary restating the piece; vary sentence and paragraph length; the article must contain a conditional decision rule, a real tradeoff, a case where the obvious answer is wrong, and a boundary condition where the whole procedure stops applying.',
...
DECISION DIMENSIONS available: ${(vertical.decision_dimensions || []).join('; ') || 'n/a'}` : '',
    ...
    '1. NUMBERS. Do not write any statistic, percentage or count that is not present in ALLOWED CLAIMS, unless the sentence itself labels it as an example or scenario ("for example", "suppose", "say a"). An unlabelled number that is not in the source is a fabrication.',
    '1b. CURRENCY. Never write a currency amount that is not in ALLOWED CLAIMS - not in a scenario, not in an example, not anywhere. A made-up price reads as a real one, and "$X per month" is refused outright by the safety gate. Describe price relationships in words: "the lower tier", "a fraction of the seat price", "less than the annual commitment".',
    ...
    '7. INSIGHT. The article must contain all four of these, and it is judged on whether they are there: (a) a conditional decision rule the reader can apply - "downgrade it when X", "if X, cancel"; (b) a real tradeoff, naming what is given up; (c) a case where the obvious recommendation is wrong, and why; (d) a boundary condition where the whole procedure stops applying.',
...
async function generate(source, identity, lens, vertical) {
  ...
  const deepenPrompt = critic
    ? `${brief}\n\nDEEPEN: Rewrite the article using the critique. Remove every unsupported number, every invented currency amount and every invented human detail. Add at least one non-obvious decision rule, one real tradeoff, and one case where the obvious recommendation should not be followed - grounded only in the allowed claims. Article only.\n\nARTICLE:\n${draft}\n\nCRITIQUE:\n${critic}`
    : `${brief}\n\nDEEPEN: Rewrite the article below. Remove every unsupported number, every invented currency amount and every invented human detail. Add at least one non-obvious decision rule, one real tradeoff, and one case where the obvious recommendation should not be followed - grounded only in the allowed claims. Article only.\n\nARTICLE:\n${draft}`;
