...
const agentNode = String.raw`const AgentNode = ({label, left, top, delay, strong = false}) => {
  ...
  return (
    <div style={{position:'absolute',left,top,width:220,height:92,borderRadius:22,border:`1.5px solid ${strong ? 'rgba(104,170,255,.72)' : C.border}`,background:strong ? 'linear-gradient(145deg,rgba(18,48,82,.97),rgba(7,18,30,.94))' : `linear-gradient(145deg,rgba(15,27,43,.94),rgba(5,11,19,.96))`,display:'flex',alignItems:'center',paddingLeft:28,gap:16,opacity:inP,translate:`${driftX}px ${driftY}px`,scale:depth,boxShadow:strong ? `0 22px 56px rgba(0,0,0,.34),0 0 34px rgba(104,170,255,.20),inset 0 1px 0 rgba(255,255,255,.10)` : `0 24px 54px rgba(0,0,0,.30),inset 0 1px 0 rgba(255,255,255,${highlight})`,backdropFilter:'blur(7px)'}}>
...
const infoCard = String.raw`const InfoCard = ({label, text, index}) => {
  ...
  const surface = index % 3 === 0
    ? 'linear-gradient(145deg,rgba(18,34,54,.96),rgba(6,13,22,.95))'
    : index % 3 === 1
      ? 'linear-gradient(155deg,rgba(11,23,38,.90),rgba(8,14,23,.98))'
      : 'linear-gradient(135deg,rgba(20,29,43,.93),rgba(5,11,19,.97))';
...
mustReplace('<SceneShell duration={sceneFrames[4]}>', '<SceneShell duration={sceneFrames[4]} intensity={1.05}>\n    <DepthAtmosphere mode="kit" />', 'Scene4 atmosphere');
...
const oldScene4 = "<div style={{height: 210, borderRadius: 24, border: `1.5px solid ${C.border}`, background: C.card, padding: 27}}>";
const newScene4 = "<div style={{height:210,borderRadius:24,border:`1.5px solid ${C.border}`,background:i%3===0?'linear-gradient(145deg,rgba(17,33,52,.96),rgba(5,12,20,.97))':i%3===1?'linear-gradient(155deg,rgba(11,24,39,.94),rgba(6,12,20,.98))':'linear-gradient(135deg,rgba(20,29,43,.92),rgba(5,11,19,.98))',padding:27,translate:`${Math.sin((useCurrentFrame()+i*14)/(30+i))*4}px ${Math.cos((useCurrentFrame()+i*9)/38)*2}px`,scale:.99+(i%2)*.006,boxShadow:'0 24px 52px rgba(0,0,0,.27),inset 0 1px 0 rgba(255,255,255,.06)',backdropFilter:'blur(8px)'}}>";
mustReplace(oldScene4, newScene4, 'Scene4 material/motion');
