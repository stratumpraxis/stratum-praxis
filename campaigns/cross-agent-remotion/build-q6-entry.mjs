...
const DepthAtmosphere = ({mode = 'neutral'}) => {
  ...
  const haze = mode === 'conflict' ? 'rgba(255,116,111,.075)' : 'rgba(104,170,255,.075)';
  ...
  return <AbsoluteFill style={{pointerEvents:'none',overflow:'hidden'}}>
    <div style={{position:'absolute',left:-150+farX,top:500+farY,width:820,height:820,borderRadius:'50%',background:'radial-gradient(circle,'+haze+',transparent 68%)',filter:'blur(34px)',opacity:.78}} />
...
const agentNode = String.raw`const AgentNode = ({label,left,top,delay,strong=false}) => {
  ...
  const border='1.5px solid '+(strong?'rgba(104,170,255,.72)':C.border);
  const shadow=strong?'0 22px 56px rgba(0,0,0,.34),0 0 34px rgba(104,170,255,.20),inset 0 1px 0 rgba(255,255,255,.10)':'0 24px 54px rgba(0,0,0,.30),inset 0 1px 0 rgba(255,255,255,'+highlight+')';
  return <div style={{position:'absolute',left,top,width:220,height:92,borderRadius:22,border,background:strong?'linear-gradient(145deg,rgba(18,48,82,.97),rgba(7,18,30,.94))':'linear-gradient(145deg,rgba(15,27,43,.94),rgba(5,11,19,.96))',display:'flex',alignItems:'center',paddingLeft:28,gap:16,opacity:inP,translate:driftX+'px '+driftY+'px',scale:depth,boxShadow:shadow,backdropFilter:'blur(7px)'}}>
...
const infoCard = String.raw`const InfoCard = ({label,text,index}) => {
  ...
  const border='1.5px solid '+(index===3?'rgba(255,116,111,.48)':C.border);
  const shadow='0 '+(18+index*2)+'px '+(42+index*3)+'px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.065)';
  return <Fade from={8+index*4} dy={18}>
    <div style={{height:205,borderRadius:24,border,background:surface,padding:'26px 28px',translate:dx+'px '+dy+'px',scale:depthScale,boxShadow:shadow,backdropFilter:'blur(8px)'}}>
...
const scene3 = String.raw`const Scene3 = () => {
  const frame=useCurrentFrame();
  const masterGlow=.22+.18*(1+Math.sin(frame/12))/2;
  const rows=[['Claude adapter','agent-specific rules'],['Codex adapter','execution boundaries'],['AGENTS.md MASTER POLICY','source of operational truth'],['Cursor adapter','coding context'],['Human Gate Matrix','approval map']];
  return <SceneShell duration={sceneFrames[3]} intensity={1.1}>
    <DepthAtmosphere mode="policy" />
    ...
    <div style={{position:'absolute',left:72,top:705,width:936,display:'flex',flexDirection:'column',gap:17}}>
      {rows.map(([left,right],i)=>{
        const dx=Math.sin((frame+i*9)/30)*(i===2?1.5:4);
        const dy=Math.cos((frame+i*13)/38)*(i===2?1:2.5);
        const surface=i===2?'linear-gradient(105deg,rgba(24,67,112,.97),rgba(6,17,29,.97))':i%2?'linear-gradient(145deg,rgba(13,25,39,.94),rgba(6,12,20,.97))':'linear-gradient(155deg,rgba(17,29,43,.92),rgba(5,11,19,.98))';
        const border=(i===2?2.5:1.5)+'px solid '+(i===2?C.blue:C.border);
        const shadow=i===2?'0 26px 62px rgba(0,0,0,.34),0 0 46px rgba(104,170,255,'+masterGlow+'),inset 0 1px 0 rgba(255,255,255,.10)':'0 22px 48px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.055)';
        return <Fade key={left} from={10+i*6} dy={20}><div style={{height:116,borderRadius:23,border,background:surface,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 28px',translate:dx+'px '+dy+'px',scale:i===2?1.012:.992+(i%2)*.004,boxShadow:shadow,backdropFilter:'blur(8px)'}}><strong style={{fontSize:29,color:i===2?'#89beff':C.white}}>{left}</strong><span style={{fontSize:18,color:C.muted}}>{right}</span></div></Fade>;
      })}
    </div>
...
`;
replaceBetween('const Scene3 = () => {','const Scene4 = () => (',scene3,'Scene3');
