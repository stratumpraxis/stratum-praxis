...
function plainCodeopsState(state){
...
async function githubEarn(){
  try{
    $('githubEngine').textContent='GitHubの有料Taskを探しています…'
    setMessage('GitHubで収益候補を探してCodeOpsを動かしています…')
    const x=await api('/api/github-earn',{method:'POST',body:'{}'})
    if(x.outcome==='GITHUB_EARN_ALREADY_RUNNING'){
      $('githubEngine').textContent='すでに探索中です'
      setMessage('GitHub収益エンジンはすでに動いています')
    }else{
      $('githubEngine').textContent='探索を開始しました'
      setMessage('GitHub収益エンジンを開始しました。明確な報酬付き案件だけを探します')
    }
    setTimeout(load,1800)
  }catch(e){
    $('githubEngine').textContent='開始できませんでした'
    setMessage('GitHub収益開始エラー: '+(e&&e.message?e.message:String(e)))
  }
}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  $('githubEarnBtn').addEventListener('click',githubEarn)
