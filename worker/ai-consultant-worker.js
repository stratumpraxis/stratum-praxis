...
function render(id,arr){document.getElementById(id).innerHTML=arr.map((x,i)=>`<label class="check"><input type="checkbox" data-w="${x[1]}" data-fix="${x[2]}"><span>${x[0]}</span></label>`).join('')};render('perm',groups.perm);render('cost',groups.cost);render('rel',groups.rel);const all=[...document.querySelectorAll('input[type=checkbox]')];function calc(){let raw=all.filter(x=>x.checked).reduce((a,x)=>a+Number(x.dataset.w),0);const score=Math.min(100,raw);document.getElementById('score').textContent=score;document.getElementById('bar').style.width=score+'%';document.getElementById('verdict').textContent=score>=60?'HIGH — hold production until blocking controls are fixed.':score>=30?'MEDIUM — remediate priority gaps before expanding autonomy.':'LOWER — no major checked red flags, but validate assumptions and monitor.';const fixes=all.filter(x=>x.checked).sort((a,b)=>Number(b.dataset.w)-Number(a.dataset.w)).map(x=>`<li>${x.dataset.fix}</li>`).join('');document.getElementById('actions').innerHTML=fixes||'<li>No checked red flags. Document approval boundaries, budget ceiling, verification, rollback and owner before production.</li>'}all.forEach(x=>x.addEventListener('change',calc));calc();document.getElementById('copy').onclick=async()=>{const t=`AI Agent Risk & Cost Audit\nRisk score: ${document.getElementById('score').textContent}/100\n${document.getElementById('verdict').textContent}\n\nPriority actions:\n${[...document.querySelectorAll('#actions li')].map((x,i)=>`${i+1}. ${x.textContent}`).join('\n')}\n\nDeployment note:\n${document.getElementById('note').value}`;await navigator.clipboard.writeText(t);document.getElementById('copy').textContent='Copied'};
</script></body></html>`;
}
